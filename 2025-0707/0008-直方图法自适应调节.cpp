...
Scalar computeHSVRangeWithKMeans(const Mat& hsv_roi, const Mat& mask, int k_clusters = 3);//聚类法计算自适应的值
...
int main() {
    ...
    if (!h_values.empty()) {
        ...
        // 调整了绿色H值的猜测范围
        int green_h_start_guess = 15/*20*/; // 扩大范围
        int green_h_end_guess = 120/*100*/;  // 扩大范围
...
Mat applyMorphology(const Mat& mask) {
    ...
    morphologyEx(result, result, MORPH_CLOSE, kernel1, Point(-1, -1), 1);
    return result;
}
...
// //===采用聚类的方法
// Scalar computeHSVRangeWithKMeans(const Mat& hsv_roi, const Mat& mask, int k_clusters = 3) {
...
//                 if (pixel[1] > 30 && pixel[2] > 50) { // 排除低饱和和亮度
//                     hsv_pixels.emplace_back(pixel[0], pixel[1], pixel[2]);
//                 }
//             }
//         }
//     }
...
//     if (hsv_pixels.empty()) {
//         cout << "未提取到有效HSV像素，使用默认值\n";
//         return Scalar(35, 43, 46); // 默认绿色下界
//     }

//     // 转换为 Mat 进行 k-means
//     Mat samples((int)hsv_pixels.size(), 3, CV_32F, hsv_pixels.data());
//     samples = samples.clone(); // 防止野指针
...
//     // 选择最可能为绿色的聚类中心
//     int green_index = -1;
//     float green_score_max = -1;
//     for (int i = 0; i < centers.rows; ++i) {
//         float h = centers.at<float>(i, 0);
//         float s = centers.at<float>(i, 1);
//         float v = centers.at<float>(i, 2);

//         // 判断绿色可能性：H在40-90之间，且S,V不能太低
//         float green_score = (h >= 40 && h <= 90 ? 1 : 0) * (s / 255.0f) * (v / 255.0f);
...
//     // 设置阈值范围（可以根据需求扩展上下限）
//     int hmin = max(0, int(h_center - 10));
//     int hmax = min(180, int(h_center + 10));
//     int smin = max(0, int(s_center - 40));
//     int smax = min(255, int(s_center + 40));
//     int vmin = max(0, int(v_center - 40));
//     int vmax = min(255, int(v_center + 40));
